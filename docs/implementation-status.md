...
- OpenAI Responses 自定义函数闭环：函数定义、JSON Schema、`tool_choice`、单函数 `function_call`、`function_call_output` 续接、能力声明和返回前参数校验；
...
- Codex 主链使用当前登录态的最小化 Responses SSE、连接复用和 macOS 系统 HTTPS 代理；普通请求保持空工具集，原生函数请求只携带调用方本轮声明的函数并从完成事件重建标准 `function_call`；函数请求不降级到无法表达动态函数的 App Server；普通请求在明确可安全回退的 HTTP 状态使用隔离的常驻 App Server，结果不明的网络错误不重放；不请求、不记录、不返回账户额度；
...
- 用户级 LaunchAgent 使用清洁环境启动，不把调用任务中的无关 Token 继承到服务进程；异常退出由 launchd 拉起，并在 macOS 重启后的用户登录阶段自动启动。

## 2026-08-31 至 2026-09-01 真实验证

- 标准 API、Codex、WorkBuddy 均成功刷新当前可用模型；分别返回 10、7、15 个模型；
- Codex Luna、Terra、Sol 均通过当前登录态直连返回声明函数及符合 JSON Schema 的参数，工具决策约 `1–3` 秒；Luna 已完成 `function_call_output` 第二轮续接；标准 API Luna、Sol 通过统一接口完成函数调用，Terra 直连协议成功但统一接口验收遇到过上游瞬时启动失败；
...
- `/v1/chat/completions`、Structured Outputs、MCP、托管工具、并行函数调用和完整 OpenAI API 覆盖；
